/**
 * The plugin configuration for every environment. It lives here, rather than in
 * `config/plugins.ts`, because Strapi Cloud rewrites that file on every deploy to
 * inject its own upload provider — which would silently drop everything below
 * (Resend email, upload security, session settings).
 *
 * Strapi loads `config/plugins` first and deep-merges this file over it, so on
 * Cloud their injected provider survives and our settings come back on top.
 * `config/plugins.ts` re-exports this file so local development, where
 * `config/env/production` is never read, gets exactly the same config.
 *
 * Keep this inside `config/env/`. Cloud's builder copies `config/*.ts` and
 * `config/env/**` and nothing else under `config/`, so a shared module in, say,
 * `config/shared/` compiles locally and then fails the Cloud build with TS2307.
 */
