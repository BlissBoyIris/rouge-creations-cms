/**
 * The real plugin configuration, kept out of `config/plugins.ts` on purpose.
 *
 * Strapi Cloud rewrites `config/plugins.ts` on every deploy to inject its own
 * upload provider, which would silently drop everything below (Resend email,
 * upload security, session settings). Nothing under `config/shared/` is touched
 * by that override, so both `config/plugins.ts` (local dev) and
 * `config/env/production/plugins.ts` (Cloud) re-export this file. The production
 * copy is deep-merged over whatever Cloud injected, so their provider survives
 * and our settings come back.
 */
